/*
  # Clean API Schema Migration with Proper Policy Management
  ...
  4. Default Data
    - Insert platform settings with proper conflict handling
*/

-- Create api schema if it doesn't exist
CREATE SCHEMA IF NOT EXISTS api;

-- Drop existing tables from public schema if they exist
DROP TABLE IF EXISTS public.report_updates CASCADE;
DROP TABLE IF EXISTS public.report_files CASCADE;
DROP TABLE IF EXISTS public.reports CASCADE;
DROP TABLE IF EXISTS public.platform_settings CASCADE;

-- Drop existing tables from api schema if they exist (for clean migration)
...
-- Drop existing policies before creating new ones (reports table)
...
-- Drop existing policies before creating new ones (report_files table)
...
-- Drop existing policies before creating new ones (report_updates table)
...
-- Drop existing policies before creating new ones (platform_settings table)
...
-- Insert default platform settings into api schema
...
CREATE TRIGGER update_reports_updated_at BEFORE UPDATE ON api.reports
    FOR EACH ROW EXECUTE FUNCTION api.update_updated_at_column();

CREATE TRIGGER update_platform_settings_updated_at BEFORE UPDATE ON api.platform_settings
    FOR EACH ROW EXECUTE FUNCTION api.update_updated_at_column();